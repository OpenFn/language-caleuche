...
function type(keys) {
  return function (state) {
    ...
    return state.element.sendKeys(parseKeys(state, array)).then(function () {
      return state;
    });
  };
}
...
function chord(keys) {
  return function (state) {
    return state.element.sendKeys(state.Key.chord(parseKeys(state, keys))).then(function () {
      return state;
    });
  };
}
...
function visible(needle, timeout) {
  return function (state) {
    return search(state, (0, _Utils.getPath)(state, needle), timeout);
  };
}
