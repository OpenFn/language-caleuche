...
function type(keys) {
  return function (state) {
    ...
    return state.driver.actions().sendKeys(parseKeys(state, array)).perform().then(function () {
      return state;
    });
  };
}
...
function chord(keys) {
  return function (state) {
    ...
    return state.driver.actions().sendKeys(state.Key.chord(parseKeys(state, keys))).perform().then(function () {
      return state;
    });
  };
}
...
function visible(needle, timeout) {
  return function (state) {
    return search(state, (0, _Utils.getPath)(state, needle), timeout).then(function () {
      return state;
    });
  };
}
...
function click(type, needle, timeout) {
  return function (state) {
    ...
    if (!needle) {
      ...
      return search(state, (0, _Utils.getPath)(state, needle), timeout).then(function (_ref) {
...
function ocr(_ref3) {
  ...
  return function (state) {
    ...
    if (mock) {
      ...
      return search(state, anchorImage).then(function (target) {
