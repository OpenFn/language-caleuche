...
function execute() {
  ...
  chromeCapabilities.set('chromeOptions', {
    'args': ['--headless', '--no-gpu', '--window-size=1920,1080']
...
function typeInElement(keys) {
  return function (state) {
    ...
    var array = typeof keys == 'string' ? [keys] : keys;
    console.log("typing: " + (0, _Utils.parseKeys)(state, array));
    return state.element.sendKeys((0, _Utils.parseKeys)(state, array)).then(sleep(state.delay)).then(function () {
      return state;
    });
  };
}
...
function assertVisible(needle, timeout, confidence) {
  return function (state) {
 ...
 * @returns {<Operation>}
 */
function click(type, needle, timeout, confidence) {
  return function (state) {
    ...
    if (!needle) {
      ...
      return Promise.race(searchArray(state, needle, timeout, confidence)).then(function (_ref) {
...
function search(state, image, timeout, confidence) {
  ...
  return (0, _promiseRetry2.default)(options, function (retry, number) {
    return state.driver.takeScreenshot().then(function (haystack, err) {
      return (0, _OpenCV.findInImage)((0, _Utils.base64_encode)(image), haystack, confidence);
    }).catch(retry);
  }).then(function (_ref2) {
