...
function imageDoubleClick(needle) {
  return function (state) {
    ...
    return (0, _promiseRetry2.default)({ factor: 1, maxTimeout: 1000 }, function (retry, number) {
      ...
      console.log(targetPos);
      state.driver.actions().mouseMove(state.element, targetPos).doubleClick().perform();
    }).then(function (data) {
      var nextState = (0, _languageCommon.composeNextState)(state, data);
      return state;
    });
...
function wait(image) {
  return function (state) {
    return (0, _promiseRetry2.default)({ factor: 1, maxTimeout: 2000 }, function (retry, number) {
      console.log('attempt number', number);
      return state.driver.findInImage((0, _Utils.getPath)(state, needle), haystack).catch(retry);
    }).then(function (data) {
      var nextState = (0, _languageCommon.composeNextState)(state, data);
      return state;
    });
