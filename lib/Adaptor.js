...
function press(key) {
  return function (state) {
    return state.element.sendKeys(_seleniumWebdriver.Key.RETURN).then(function (data) {
      return (0, _languageCommon.composeNextState)(state, data);
    });
  };
}
...
function singleClick(state, target) {
  ...
  return state.driver.actions().mouseMove(state.element, targetPos).click().perform();
}

function doubleClick(state, target) {
  console.log("in the d-click function");
  return state.driver.actions().mouseMove(state.element, targetPos).doubleClick().perform();
}

function wait(needle) {
  return function (state) {
    return (0, _promiseRetry2.default)({ factor: 1, maxTimeout: 1000 }, function (retry, number) {
      return state.driver.takeScreenshot().then(function (haystack, err) {
        return (0, _OpenCV.findInImage)((0, _Utils.getPath)(state, needle), haystack).catch(retry);
