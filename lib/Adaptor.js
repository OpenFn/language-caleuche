...
function type(keys) {
  return function (state) {
    ...
    return state.element.sendKeys((0, _Utils.parseKeys)(state, array)).then(function () {
      return state;
    });
  };
}
...
function chord(keys) {
  return function (state) {
    ...
    return state.element.sendKeys(state.Key.chord((0, _Utils.parseKeys)(state, keys))).then(function () {
      return state;
    });
  };
}
